...
class ProductRepo {
  ...
  Future<ApiResponse> getPopularProductList(
    ...
    try {
      final response = await dioClient.get(
        '${AppConstants.POPULAR_PRODUCT_URI}?limit=10&&offset=$offset&cat_id=$categoryId',
  ...
  Future<ApiResponse> getLatestProductList(
    ...
    try {
      final response = await dioClient.get(
        '${AppConstants.LATEST_PRODUCT_URI}?limit=10&&offset=$offset&cat_id=$categoryId',
        options: Options(headers: {'X-localization': languageCode}),
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
  ...
  Future<ApiResponse> getDailyItemList(
    ...
    try {
      final response = await dioClient.get(
        '${AppConstants.DAILY_ITEM_URI}?limit=10&&offset=$offset&cat_id=$categoryId',
        options: Options(headers: {'X-localization': languageCode}),
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
